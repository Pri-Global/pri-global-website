/** Country dial codes for phone inputs — 234 countries, US default. */
...
export function formatCountryOption(country) {
  return `${country.flag} ${country.name} (${country.dial})`;
}

export function formatCountryCompact(country) {
...
export function parsePhoneValue(value = "") {
  ...
  for (const country of byDial) {
    if (trimmed.startsWith(country.dial)) {
      return {
        ...
        national: trimmed.slice(country.dial.length).replace(/\D/g, ""),
      };
    }
  }
  ...
  if (digits.startsWith("1") && digits.length === 11) {
    return { countryCode: "US", national: digits.slice(1) };
  }

  return { countryCode: DEFAULT_PHONE_COUNTRY, national: digits };
}

export function formatPhoneValue(countryCode, national = "") {
